    ...
    async def verify_user(
        ...
        try:
            ...
            if db_user.email not in verify_codes:
                ...
                return {"error": "No verification code found for this user", "status": 400}
            ...
            if isinstance(stored_data, dict):
                ...
                if stored_data.get("code") != code:
                    logger.error(f"Invalid verification code for user_id {user_id}, provided: {code}")
                ...
                logger.debug("Using string stored_data, device_id will be generated from request")
            ...
            if device_id is None:
                ...
                if request:
                    try:
                        ...
                        logger.error(f"Error generating device_id from request: {str(e)}")
                        # Fallback
                        import uuid
                        device_id = f"fallback_{uuid.uuid4().hex}"
                        device_info = {"error": "Device detection failed"}
