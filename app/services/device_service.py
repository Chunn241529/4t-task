...
class DeviceService:
    ...
    def add_verified_device(db: Session, user_id: int, device_id: str, device_info: Optional[Dict] = None) -> bool:
        ...
        try:
            ...
            existing_devices = [d.get('device_id') if isinstance(d, dict) else d 
                              for d in user.verified_devices]
            existing_devices = [d for d in existing_devices if d]
            
            if normalized_device_id in existing_devices:
                logger.debug(f"Device {normalized_device_id} already verified for user {user_id}")
                return True
            
            # SỬA: Sử dụng datetime.now(timezone.utc) thay vì datetime.utcnow()
            current_time = datetime.now(timezone.utc).isoformat()
            
            # Tạo device entry với thông tin chi tiết
            device_entry = {
                "device_id": device_id,
                "verified_at": current_time,  # SỬA Ở ĐÂY
            ...
            logger.info(f"Added device {device_id} to verified devices for user {user_id}")
            return True
    ...
    def is_device_verified(db: Session, user_id: int, device_id: str) -> bool:
        ...
        try:
            ...
            existing_devices = [d for d in existing_devices if d]
            normalized_device_id = device_id.strip().lower()
            existing_devices_normalized = [d.strip().lower() for d in existing_devices]
            
            return normalized_device_id in existing_devices_normalized
    ...
    def remove_verified_device(db: Session, user_id: int, device_id: str) -> bool:
        ...
        try:
            ...
            user.verified_devices = [
                ...
                if (isinstance(d, dict) and d.get('device_id', '').strip().lower() != normalized_device_id) or
                   (not isinstance(d, dict) and d.strip().lower() != normalized_device_id)
            ]
